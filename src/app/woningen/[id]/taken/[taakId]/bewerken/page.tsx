...
import { getTaakById } from "@/services/taken";
...
export default async function TaakBewerkenPage({
  params,
}: Props) {
  const { id, taakId } = await params;
  const woningId = Number(id);
  const taakNummer = Number(taakId);

  if (
    !Number.isInteger(woningId) ||
    woningId <= 0 ||
    !Number.isInteger(taakNummer) ||
    taakNummer <= 0
  ) {
    notFound();
  }

  const [woning, taak] = await Promise.all([
    getWoningById(woningId),
    getTaakById(taakNummer),
  ]);

  if (!woning || !taak || taak.woning_id !== woning.id) {
    notFound();
  }
    ...
    <main className="min-h-screen bg-slate-100 px-4 py-6 text-slate-950 sm:px-6 md:px-8 md:py-10">
      <div className="mx-auto max-w-4xl">
        ...
        <section className="mt-6 rounded-3xl bg-white p-6 shadow-sm md:p-8">
          ...
          <div className="mt-8">
            <TaakForm woningId={woning.id} taak={taak} />
          </div>
        </section>
      </div>
    </main>
