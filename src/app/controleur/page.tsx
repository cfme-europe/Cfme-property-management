...
export default async function ControleurPage() {
  ...
  return <main className="min-h-screen bg-slate-100 px-4 py-5 text-slate-900 sm:px-6 md:py-8"><div className="mx-auto max-w-5xl"><header className="rounded-3xl bg-slate-950 p-6 text-white shadow-lg md:p-8"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-semibold uppercase tracking-widest text-emerald-300">CFME Control</p><h1 className="mt-2 text-3xl font-bold">Mijn controles</h1><p className="mt-2 text-slate-300">{naam}</p></div></div><div className="mt-6 rounded-2xl bg-slate-800 p-4"><p className="text-sm text-slate-300">Nu aandacht nodig</p><p className="mt-1 text-3xl font-bold">{actielijst.length}</p></div></header>{actielijst.length === 0 ? <section className="mt-6 rounded-2xl bg-white p-7 shadow-sm"><h2 className="text-xl font-bold">Geen controles open</h2><p className="mt-2 text-slate-600">Er zijn geen gestarte, achterstallige of binnen zeven dagen geplande controles aan jou toegewezen.</p></section> : <div className="mt-6 space-y-5">{actielijst.map((woning) => { const woningStatus = status(woning); return <article key={woning.woning_id} className="rounded-2xl bg-white p-5 shadow-sm md:p-6"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-sm font-semibold text-emerald-700">{woning.rayon_naam} · {woning.rayon_code}</p><h2 className="mt-1 text-2xl font-bold">{woning.adres}</h2><p className="mt-1 text-slate-600">{woning.postcode} {woning.plaats}</p></div><span className={`rounded-full px-3 py-1 text-sm font-semibold ${woningStatus.klasse}`}>{woningStatus.label}</span></div><dl className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><div className="rounded-xl bg-slate-100 p-4"><dt className="text-sm text-slate-500">Laatste controle</dt><dd className="mt-1 font-bold">{datum(woning.laatste_inspectiedatum)}</dd></div><div className="rounded-xl bg-slate-100 p-4"><dt className="text-sm text-slate-500">Volgende controle</dt><dd className="mt-1 font-bold">{datum(volgendeControle(woning).toISOString().slice(0,10))}</dd></div><div className="rounded-xl bg-slate-100 p-4"><dt className="text-sm text-slate-500">Urgente meldingen</dt><dd className="mt-1 font-bold">{woning.urgente_meldingen}</dd></div><div className="rounded-xl bg-slate-100 p-4"><dt className="text-sm text-slate-500">Frequentie</dt><dd className="mt-1 font-bold">{woning.controlefrequentie_dagen} dagen</dd></div></dl>{woning.controlesessie_status === "bezig" && woning.open_inspectie_id ? <div className="mt-5 rounded-xl border border-blue-200 bg-blue-50 p-4"><p className="font-bold text-blue-900">Controle gestart: {datumTijd(woning.gestart_at)}</p><p className="mt-1 text-sm text-blue-800">{locatieLabel(woning.locatie_status)}</p><Link href={`/controleur/controle/${woning.controlesessie_id}`} className="mt-4 block rounded-xl bg-blue-700 px-5 py-4 text-center font-bold text-white">Doorgaan met controle</Link></div> : <div className="mt-5"><ControleStartButton woningId={woning.woning_id} verhuurperiodeId={woning.verhuurperiode_id} controleurId={werkplek.profiel.id} controleurNaam={naam} openInspectieId={woning.open_inspectie_id} /></div>}</article>; })}</div>}</div></main>;
}
